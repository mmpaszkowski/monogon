...
struct AddOperation<T, Matrix<U>, Vector<V>> : public Operation {
    ...
    perform(const std::any &grad, const std::any &lhs, const std::any &rhs) const override {
...
struct DivOperation : public Operation {
    ...
    perform(const std::any &grad, const std::any &lhs, const std::any &rhs) const override {
        ...
        return std::tuple(
                grad_val * (lhs_initializer.initialize(lhs_val, 1.0) / rhs_val),
...
//        rhs_type rhs_value = std::any_cast<rhs_type>(rhs);
        ...
        Matrix<U> l_result = Matrix<U>(lhs_value.get_rows(), lhs_value.get_columns(), 1.0 / (lhs_value.get_rows() * lhs_value.get_columns())) * grad_value;
        return std::tuple(l_result, std::any());
    }
};
