...
template <typename T = double> class DenseNode : public LayerNode<T>
{
  ...
  private:
    ...
    std::shared_ptr<Initializer<T>> initializer;
    Variable<Matrix<T>> weights;
    Variable<Matrix<T>> bias;
};
...
DenseNode<T>::DenseNode(std::vector<size_t> shape,
    ...
    : shape(shape), initializer(initializer), weights(initializer->operator()(layer->get_shape()[0], shape[0])),
      bias(Matrix<T>(1, this->shape[0], 0.0))
{
...
template <typename T> Variable<Matrix<T>> DenseNode<T>::feed_forward(const Variable<Matrix<T>> &X)
{
...
template <typename T> Variable<Matrix<T>> Dense<T>::feed_forward(const Variable<Matrix<T>> &X)
{
